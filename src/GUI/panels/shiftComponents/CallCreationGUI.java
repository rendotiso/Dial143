...
public class CallCreationGUI {
    ...
    // ── Sticky Note hint colors ───────────────────────────────────────────────
    public static final Color HINT_FILL   = new Color(255, 245, 180);      
    public static final Color HINT_BORDER = new Color(210, 170,  30);       
    public static final Color HINT_TEXT   = new Color(130, 100,   0);     
    ...
    public static final Font HINT_FONT     = new Font("Segoe UI", Font.BOLD,  12);
    ...
    public static int drawDescription(Graphics2D g2, String callerName,
                                       String displayedDesc, int y) {
        ...
        for (String line : displayedResponse.split("\n")) {
            ...
            if (line.startsWith("You:")) {
                ...
                for (String part : parts) {
                    ...
                    if (part.contains("+Performance Points")) g2.setColor(PP_COLOR);
                    else if (part.contains("+Salary"))        g2.setColor(SALARY_COLOR);
    ...
    public static void drawTimerBar(Graphics2D g2, int timerLeft, int timerMax, int timerBoostDisplay) {
        int barX = BOX_X;
        int barW = BOX_W;
        ...
        g2.setColor(fill);
        String timeText = timerLeft + "s";
        FontMetrics fm = g2.getFontMetrics();
        g2.drawString(timeText, barX + barW - fm.stringWidth(timeText) - 12, TIMER_Y - 8);

        if (timerBoostDisplay > 0) {
            g2.setFont(new Font("Segoe UI", Font.BOLD, 12));
            g2.setColor(new Color(50, 150, 50));
            String boostText = "+" + timerBoostDisplay + "s";
            int boostX = barX + 10;
            int boostY = TIMER_Y - 5;
            g2.drawString(boostText, boostX, boostY);
        }
    }

    public static void drawTimerBar(Graphics2D g2, int timerLeft, int timerMax) {
        drawTimerBar(g2, timerLeft, timerMax, 0);
    }
...
public static void drawChoices(Graphics2D g2, String[] labels, int chosenIndex,
        ...
        for (int i = 0; i < count; i++) {
            ...
            boolean isHint    = hintIndex == i && chosenIndex == -1;

            choiceRects[i] = new Rectangle(bx, CHOICE_Y, btnW, CHOICE_H);

            // ── Fill ─────────────────────────────────────────────────────────
            if (isHint) {
                g2.setColor(HINT_FILL);
            } else {
            ...
            // ── Border ───────────────────────────────────────────────────────
            g2.setStroke(new BasicStroke(isHint ? 2.5f : 2f));
            g2.setColor(isHint ? HINT_BORDER : CHOICE_BORDER);
    ...
    /**
     * Overload kept for backward compatibility — no hint shown.
     */
    public static void drawChoices(Graphics2D g2, String[] labels, int chosenIndex,
                                    int hoveredChoice, Rectangle[] choiceRects) {
        drawChoices(g2, labels, chosenIndex, hoveredChoice, choiceRects, -1);
    }
    ...
    private static Font loadFont(float size, int style) {
        try   { return new Font("Segoe UI", style, (int) size); }
